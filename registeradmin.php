...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password !== $confirmPassword) {
        header('Location: registeradmin.php?status=error&message=Passwords do not match');
        exit;
    }
    ...
    if ($stmt->fetch(PDO::FETCH_ASSOC)) {
        header('Location: registeradmin.php?status=error&message=Username already exists');
        exit;
    }
    ...
    // Debugging line to check values being inserted
    error_log("Inserting values: Username: $username, Email: $email, User Type: $user_type"); 

    // Insert into the database, including user_type
    ...
    if ($stmt->execute()) {
        header('Location: registeradmin.php?status=success&message=Welcome, ' . urlencode($username));
    } else {
        header('Location: registeradmin.php?status=error&message=An error occurred while registering your account');
    }
}
?>
...
<body>
    <div class="register-container">
        ...
        <form id="registerForm" action="registeradmin.php" method="post">
            <div class="row form-row">
                ...
                <div class="input-field" style="display: none;">
                    <input type="hidden" id="registerDeviceNumber" name="deviceNumber" value="ADMIN">
                    <input type="hidden" id="registerUserType" name="user_type" value="admin"> <!-- Modify as necessary -->
                </div>
            </div>
